...
$conditions = sprintf(" where DATEDIFF( departure_date, arrival_date ) >= %s and status=%s ",$sVisit,DETECTED);
if($sName) {
...
$query = sprintf("SELECT `name`,father,mother,dob,i.passport_no AS passport_no,purpose,country,country_last,address_bd,visa_type,occupation,status,detect_date,action_date,action, COUNT(i.id) as TotalArrival FROM fp_information i INNER JOIN suspects s ON i.passport_no=s.passport_no %s GROUP BY passport_no ORDER BY TotalArrival DESC;", $conditions);
